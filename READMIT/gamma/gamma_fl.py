...
df_raw["train"] = np.where(df_raw["CMS_2018_164"] == 1, 1, 0) # Create categorical test target outcome based on conditions
df_raw["test"] = np.where(df_raw["CMS_2018_25"] > 15.6, 1, 0) # Create categorical test target outcome based on conditions
df_raw["test2"] = np.where(df_raw["CMS_2018_25"] < 15.6, 1, 0) # Create categorical test target outcome based on conditions
...
text_file.write("       test2 = " + str(i_test2) + "\n\n") # Add two lines of blank text at end of every section text
text_file.write("####################" + "\n\n")
text_file.close() # Close file
...
text_file.write("       test2 = " + str(f_test2) + "\n\n") # Add two lines of blank text at end of every section text
text_file.write("####################" + "\n\n")
text_file.close() # Close file

# Step 5: Final Prediction with Hand Selected Predictors
sub5 = "Step 5: Final Prediction with Hand Selected Predictors"

### Hand Build Regression Model
final = ["DP03_0012PE_gini",
                "CMS_2018_5",
                "CMS_2018_9",
                "CMS_2018_10",
                "CMS_2018_11",
                ...
                "CMS_2018_158",
                "CMS_2018_160"]
df_h = pd.merge(X_g_raw, df_raw, on = "Facility ID", how = "inner") # Join by column while keeping only items that exist in both, select outer or left for other options
df_h = pd.merge(X_c_raw, df_h, on = "Facility ID", how = "inner") # Join by column while keeping only items that exist in both, select outer or left for other options
df_h = df_h.set_index("Facility ID") # Set column as index
df_h = df_h[final]
df_h = df_h.dropna()
Y = df_h["quant"]
X = df_h.drop(columns = ["quant", "train", "test", "test2"])
mod = sm.OLS(Y, X) # Describe linear model
res_h = mod.fit() # Fit model

### Join Codes and Labels
codes = pd.DataFrame()
codes["Code"] = df_h.columns
h_label = pd.merge(codes, df_label, on = "Code", how = "left") # Join by column while keeping only items that exist in both, select outer or left for other options

## Mutli-Layer Perceptron for Indetified Final Predictors

### Build Network with keras Sequential API
# Prep Inputs
ss_h = pd.DataFrame(StandardScaler().fit_transform(df_h.values), columns = df_h.columns) # Standard scale values by converting the normalized features into a tabular format with the help of DataFrame.
Y_train = ss_h["train"]
Y_test = ss_h["test"]
Y_test2 = ss_h["test2"]
X = ss_h.drop(columns = ["quant", "train", "test", "test2"])
input = X.shape[1] # Save number of columns as length minus quant, test, train and round to nearest integer
nodes = round(input / 2) # Number of input dimensions divided by two for nodes in each layer
network = Sequential()
# Dense Layers
...
final = network.fit(X, Y_train, batch_size = 10, epochs = 200) # Fitting the data to the train outcome
# Predict
Y_h = network.predict(X) # Predict values from testing model
# AUC Score
Y_pred = (Y_h > 0.5)
Y_train = (Y_train > 0)
Y_test = (Y_test > 0)
Y_test2 = (Y_test2 > 0)
fpr, tpr, threshold = roc_curve(Y_train, Y_pred) # Create ROC outputs, true positive rate and false positive rate
h_train = auc(fpr, tpr) # Plot ROC and get AUC score
fpr, tpr, threshold = roc_curve(Y_test, Y_pred) # Create ROC outputs, true positive rate and false positive rate
h_test = auc(fpr, tpr) # Plot ROC and get AUC score
fpr, tpr, threshold = roc_curve(Y_test2, Y_pred) # Create ROC outputs, true positive rate and false positive rate
h_test2 = auc(fpr, tpr) # Plot ROC and get AUC score

### Append to Text File
text_file = open(path + day + "_results" + label + ".txt", "a") # Open text file and name with subproject, content, and result suffix
text_file.write(sub5 + "\n\n") # Line of text with space after
text_file.write(f2 + "\n") # Add two lines of blank text at end of every section text
text_file.write("   Models = " + m6 + "\n") # Add two lines of blank text at end of every section text
text_file.write("   Outcome = quant, 2020 VBP Adjustment Factor" + "\n")
text_file.write(str(res_h.summary())  + "\n\n") # Add two lines of blank text at end of every section text
text_file.write(str(h_label)  + "\n\n") # Add two lines of blank text at end of every section text
text_file.write(f2 + "\n") # Add two lines of blank text at end of every section text
text_file.write("   Network = " + m1 +"\n") # Add two lines of blank text at end of every section text
text_file.write("   Layers = Dense, Dense, Activation" + "\n") # Add two lines of blank text at end of every section text
text_file.write("   Functions = ReLU, ReLU, Sigmoid" + "\n") # Add two lines of blank text at end of every section text
...
text_file.write("   Targets = (train, test, test2), (FY2020, FY2019, FY2018)" + "\n")
text_file.write("   AUC Scores" + "\n") # Add two lines of blank text at end of every section text
text_file.write("       train = " + str(h_train) + "\n") # Add two lines of blank text at end of every section text
text_file.write("       test = " + str(h_test) + "\n") # Add two lines of blank text at end of every section text
text_file.write("       test = " + str(h_test2) + "\n\n") # Add two lines of blank text at end of every section text
text_file.write("####################" + "\n\n")
text_file.close() # Close file