...
df_cms = df_cms.filter(["Medicare hospital spending per patient (Medicare Spending per Beneficiary)", "FIPS"]) # Keep only selected columns
df_join = pd.merge(df_doh, df_cms, on = "FIPS", how = "inner") # Join by column while keeping only items that exist in both, select outer or left for other options
df_cms = 0 # Clear variable
df_doh = 0 # Clear variable
...
df_man = df_man.rename(columns = {"Medicare hospital spending per patient (Medicare Spending per Beneficiary)": "outcome"}) # Rename multiple columns in place
...
x = df_NA.values # Save feature values as x
...
df_NA = pd.DataFrame(x, columns = df_NA.columns) # convert the normalized features into a tabular format with the help of DataFrame.
...
df_pc_gini_reg.to_csv(r"fp/MSPB/FL/MSPB_FL_DOH_FIPS_alpha.csv") # Export df as csv
...
X = df_step3.filter(rfe_features) # Keep only selected columns from rfe
Y = df_step3["outcome"] # Add outcome variable
